    ...
    try:
        ...
        # 处理消息事件
            ...
            if message_type == "group":
                # 调用QFNUBustExamClassroomFind的群组消息处理函数
                await handle_QFNUBustExamClassroomFind_group_message(websocket, msg)
        ...
        logging.error(f"处理QFNUBustExamClassroomFind{error_type}事件失败: {e}")
        ...
        if post_type == "message":
            ...
            if message_type == "group":
                await send_group_msg(
                    websocket,
                    msg.get("group_id"),
                    f"处理QFNUBustExamClassroomFind{error_type}事件失败，错误信息：{str(e)}",
                )
            elif message_type == "private":
